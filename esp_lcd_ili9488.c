...
typedef struct {
    ...
    uint8_t data[16];
    uint8_t data_bytes;
} lcd_init_cmd_t; // Length of data in above data array; 0xFF = end of cmds.
...
static esp_err_t panel_ili9488_del(esp_lcd_panel_t *panel) {
    ...
    if (ili9488->reset_gpio_num >= 0) gpio_reset_pin(ili9488->reset_gpio_num);
...
esp_err_t esp_lcd_new_panel_ili9488(const esp_lcd_panel_io_handle_t io, const esp_lcd_panel_dev_config_t *panel_dev_config, const size_t buffer_size, esp_lcd_panel_handle_t *ret_panel) {
    ...
    if (panel_dev_config->bits_per_pixel == 16) {
        ...
        ESP_GOTO_ON_FALSE(buffer_size > 0, ESP_ERR_INVALID_ARG, err, TAG, "Color conversion buffer size must be specified");
        ...
        ESP_GOTO_ON_FALSE(ili9488->color_buffer, ESP_ERR_NO_MEM, err, TAG,"Failed to allocate DMA color conversion buffer");
    }
